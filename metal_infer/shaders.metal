...
// ============================================================================
// Kernel 1g: 4-bit dequant matvec for HIDDEN_DIM=7168 inputs (v7k)
// ============================================================================
// Same tiled structure as v3 but uses half-precision x_shared to fit
// 7168 elements in 14KB threadgroup memory (well within 32KB Metal limit).
// For in_dim=7168: packed_cols=896, 32 SIMD lanes → exactly 28 cols per lane.
// Uses 512 threads (16 SIMD groups, 16 rows/TG) so 2 TGs/core × 512 = 1024
// threads/core = 32 warps = 100% GPU thread occupancy despite 14KB shared.
// 4x fewer dispatches vs 256-thread variant (512 vs 1024 TGs for Q proj).
// Applied to: Q/K/V, QKV, z, gate_proj, up_proj (all in_dim=HIDDEN_DIM=7168).
#define ROWS_PER_TG_7K 16

kernel void dequant_matvec_4bit_v7k(
    device const uint32_t* W_packed   [[buffer(0)]],
    device const uint16_t* scales     [[buffer(1)]],
    device const uint16_t* biases     [[buffer(2)]],
    device const float*    x          [[buffer(3)]],
    ...
    uint row = tgid * ROWS_PER_TG_7K + simd_group;

    uint packed_cols = in_dim / 8;
    uint num_groups  = in_dim / group_size;

    // Cache x in half precision: 7168 * 2 = 14KB threadgroup memory (vs 32KB limit).
    // Half precision is sufficient for normalized activations (~bf16 range).
    // ALL 512 threads participate in load before barrier — required for correctness.
    threadgroup half x_shared[7168];
    for (uint i = lid; i < in_dim; i += 512) {
        x_shared[i] = half(x[i]);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (uint col = simd_lane; col < packed_cols; col += 32) {
        ...
        float sx0 = scale * float(x_shared[x_base+0]); float bx0 = bias * float(x_shared[x_base+0]);
        float sx1 = scale * float(x_shared[x_base+1]); float bx1 = bias * float(x_shared[x_base+1]);
        float sx2 = scale * float(x_shared[x_base+2]); float bx2 = bias * float(x_shared[x_base+2]);
        float sx3 = scale * float(x_shared[x_base+3]); float bx3 = bias * float(x_shared[x_base+3]);
        float sx4 = scale * float(x_shared[x_base+4]); float bx4 = bias * float(x_shared[x_base+4]);
        float sx5 = scale * float(x_shared[x_base+5]); float bx5 = bias * float(x_shared[x_base+5]);
        float sx6 = scale * float(x_shared[x_base+6]); float bx6 = bias * float(x_shared[x_base+6]);
        float sx7 = scale * float(x_shared[x_base+7]); float bx7 = bias * float(x_shared[x_base+7]);
